# project_telephonic/models/asr.py
import logging
from transformers import pipeline
from project_telephonic.config import (
    DEVICE, DEFAULT_WHISPER_MODEL_ID,
    DISTIL_WHISPER_MODEL_ID, SELECTED_WHISPER_MODEL
)
...
class ASRModel:
    ...
    def load_model(self):
        if self.model_key == "distil":
            model_id = DISTIL_WHISPER_MODEL_ID
        else:
    ...
    def transcribe(self, wav_file: str, english_only: bool = False):
        # "generate_kwargs" is optional or can be used for language
        generate_kwargs = {}
        if english_only:
